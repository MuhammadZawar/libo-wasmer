...
use wasmer::{imports, wat2wasm, Instance, Module, Store, Value, Type, Mutability};
...
"#
    ...
    assert_eq!(result.expect_err("Expected an error").message(), "Attempted to set an immutable global");
...
#[test]
fn test_exported_globals() -> Result<(), Box<dyn std::error::Error>> {
    main()
}